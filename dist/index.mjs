...
function Queue(name) {
  return (target) => {
    SetMetadata(QUEUE_METADATA_KEY, { name })(target);
  };
}
function Processor(queue, options) {
  return (target) => {
    SetMetadata(PROCESSOR_METADATA_KEY, { queue, concurrency: options?.concurrency })(target);
  };
}
...
QueueService = __decorateClass([
  Injectable(),
  __decorateParam(1, Inject(SMART_QUEUE_MODULE_OPTIONS))
], QueueService);
...
export { BullMQAdapter, PROCESSOR_METADATA_KEY, PROCESS_HANDLER_METADATA_KEY, Process, Processor, QUEUE_METADATA_KEY, QUEUE_REGISTRY_TOKEN, QUEUE_SERVICE_TOKEN, Queue, QueueRegistry, QueueService, SMART_QUEUE_MODULE_OPTIONS, SmartQueueModule, getProcessHandlerMetadata, getProcessorMetadata, getQueueMetadata };
